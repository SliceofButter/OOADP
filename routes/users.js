...
router.get('/bio', ensureAuthenticated, function (req, res, next) {
  User.findOne({ username: req.user.username }, function (err, user) {
    Bank.findOne({ username: req.user.username }, function (err, bank) {
      imger = 'otpauth://totp/SecretKey?secret=' + user.authNo
      console.log(imger)
      QRCode.toDataURL(imger, function (err, image_data) {
...
router.post('/authy', (req, res) => {
  User.findById(req.user, function (err, user) {
    ...
    console.log(verified)
    if (verified === true) {
      User.findOneAndUpdate({ username: user.username }, { $set: { is2Fa: true } }, function (err) {
        res.redirect('/bio');
      })
    }
  })
